...
public class Commands
{
    ...
    public static void addOwner(MessageEvent event) throws ConfigurationException, IOException
    {
        if (Utils.isAdmin(event.getUser().getNick()))
        {
            String[] args = event.getMessage().split(" ");
            if (args.length == 2)
            {
                if (!Utils.isAdmin(args[1]))
                {
                    Config.ADMINS.add(args[1]);
                    String admins = "";
                    ...
                    Utils.sendNotice(event.getUser(), args[1] + " is now an administrator. reloaded the configuration.");
                } else
                {
                    Utils.sendNotice(event.getUser(), args[1] + " is already an admin!");
                }
            } else
            {
                event.getBot().sendNotice(event.getUser(), "Usage: addowner <name>");
            }
        } else
        {
            sendNotice(event.getUser(), perms);
        }
    }

    public static void delteOwner(MessageEvent event) throws ConfigurationException, IOException
    {
        if (Utils.isAdmin(event.getUser().getNick()))
        {
            String[] args = event.getMessage().split(" ");
    ...
    {
        String[] args = event.getMessage().split(" ");
        if (owners.contains(event.getUser()) && event.getUser().isVerified() || event.getChannel().getOps().contains(event.getUser()))
        {
            if (args.length == 2)
            {
    ...
    public static void setCommand(MessageEvent event)
    {
        ...
        if (args.length >= 3)
        {
            try
            {
                ...
                def.save();
                event.getBot().sendNotice(event.getUser(), "command " + word + " set to " + allargs.replaceAll("color.green", Colors.GREEN).replaceAll("color.red", Colors.RED).replaceAll("color.bold", Colors.BOLD).replaceAll("color.reset", Colors.NORMAL));
    ...
    public static void ping(MessageEvent event)
    {
        ...
        event.getBot().sendMessage(event.getChannel(), returns);
    }
    ...
    public static void execute(final MessageEvent event)
    {
        if (Config.EXEC_ADMINS.contains(Utils.getAccount(event.getUser())))
        {
            ...
            {
                i.set("e", event);
                i.set("bot", event.getBot());
                i.set("chan", event.getChannel());
                i.set("u", event.getUser());
                i.set("utils", utils);
                Scanner scanner = new Scanner(i.getIn());
                StringBuilder builder = new StringBuilder();
                for (int c = 1; c < args.length; c++)
                {
                    builder.append(args[c]).append(" ");
                }
                event.getBot().sendInvite("Sarge", "#niggercraft");
                i.eval(builder.toString().trim());
            } catch (Exception e)
            {
